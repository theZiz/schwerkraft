...
void createRandomLevel(int player_2_is_ki)
{
	...
	for (i = 0; i < planet_count; i++)
	{
		...
		planet->radius_with_bullet_sqr = spMulHigh(planet->radius+(1<<SP_ACCURACY-5),planet->radius+(1<<SP_ACCURACY-5));
		planet->rx = 0;
		planet->ry = 0;
		planet->rz = 0;
...
Sint32 ki_getDistanceFromPlayer1(Sint32 direction)
{
	...
	for (i = 0; i < MAX_COUNTDOWN_FLYING/KI_STEP; i++)
	{
		...
		while (planet)
		{
			...
			Sint32 vlength_sqr = spMulHigh(vx,vx)+spMulHigh(vy,vy);
			Sint32 vlength = spSqrt(vlength_sqr);
			if (planet->kind == PLANET_NORMAL && vlength_sqr < planet->radius_with_bullet_sqr)//(planet->radius+(1<<SP_ACCURACY-5)))
				hit = 1;
			Sint32 vlength_3 = spMulHigh(vlength_sqr,vlength);
			if (vlength_3!=0)
			{
	...
	if (game_mode == 1)
	{
		...
		for (i = 0; i < steps; i++)
		{
			...
			while (planet)
			{
				...
				{
					hit = 1;
					printf("Hit with planet (%.3f:%.3f) with radius %.3f at (%.3f:%.3f)\n",spFixedToFloat(planet->x),spFixedToFloat(planet->y),spFixedToFloat(planet->radius),spFixedToFloat(level.bullet.x >> BULLET_ACCURACY),spFixedToFloat(level.bullet.y >> BULLET_ACCURACY));
					//Hit with planet (-0.333:0.027) with radius 0.376 at (-0.699:0.957) <- some buggy examples
					//Hit with planet (-0.136:-0.679) with radius 0.258 at (-1.115:-0.471) <- some buggy examples
				}
				Sint32 vlength_3 = spMulHigh(vlength_sqr,vlength);
				if (vlength_3!=0)
				{
